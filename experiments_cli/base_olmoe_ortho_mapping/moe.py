...
from helpers.moe_utils import prepare_4d_causal_attention_mask_with_cache_position, load_balancing_loss_func, differentiable_gram_schmidt
...
class OlmoeMoe(nn.Module):
    ...
    def __init__(self, conf: ModelConf):
        ...
        # Orthogonalize the gate weights
        self.keep_magnitude = conf.keep_magnitude
        self.use_random_order = conf.use_random_order
    ...
    def forward(self, hidden_state: torch.Tensor, moe_method: str, use_lflb: bool = False) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        # Orthogonalize the gate weights
        self.orthogonal_gate_weights = differentiable_gram_schmidt(self.gate.weight, 
                                                              keep_magnitude=self.keep_magnitude, use_random_order=self.use_random_order)
    ...
    def forward_slow(self, hidden_state: torch.Tensor, use_lflb: bool) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        router_logits = F.linear(hidden_state, self.orthogonal_gate_weights)
