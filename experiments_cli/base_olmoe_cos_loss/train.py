...
def train(model, tokenizer, train_conf, model_conf, val_dl, seed):
    """
    ...
    for shard_idx, shard_path in enumerate(shard_files):
        ...
        for batch_idx, batch in enumerate(shard_dl):
            ...
            cos_loss, cos_loss_per_layer = cos_loss_func(model, model_conf) # cos_loss_per_layer is a list of tensors, one per layer
            weighted_cos_loss = train_conf.router_cos_loss_coef * cos_loss
            ...
            weighted_cos_loss.backward()
            cos_bwd_time = time.time() - start_cos_bwd
            total_bwd_time += cos_bwd_time
            total_loss_val += weighted_cos_loss.item() * train_conf.accumulation_steps # to match the average operation later
            ...
            metrics = {
                ...
                'train': {
                    ...
                    'cos_loss':  cos_loss.detach().cpu().item()
                },
            ...
            if step % 10 == 0:
                ...
                cos_dict_final = {}
                for layer_idx, cos_loss_layer in enumerate(cos_loss_per_layer):
                    cos_dict_final[layer_idx] = cos_loss_layer.detach().cpu().item()
                metrics['cos_loss'] = cos_dict_final
