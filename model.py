...
class MoE(nn.Module):
    ...
    def __init__(self,config):
        ...
        self.gate = nn.Linear(self.n_embd, self.n_experts, bias=config.bias)
        self.softmax = nn.Softmax(dim=-1)
        self.dropout = nn.Dropout(config.dropout) 
        self.main_device = config.main_device
              
    def _move_experts(self):
    ...
    def forward(self, hidden_state):
        ...
        router_logits = self.gate(hidden_state)
        ...
        expert_assignment_mask = torch.nn.functional.one_hot(selected_experts, num_classes = self.n_experts).permute(2, 1, 0) # Creates (N_EXPERTS, TOP_K, BN)
        ...
        for expert_ix, expert in enumerate(self.experts):
            expert_device = next(expert.parameters()).device # need to ensure all parameters are on the same device
            ...
            expert_output = expert(tokens_for_expert.to(expert_device))
            expert_output = expert_output.to(self.main_device) * routing_weights[token_indices, topk_slot].unsqueeze(1) # For each num_assigned_tokens, multiples it by the corresponding weight in topk_slot fort that token_index
        ...
        return hidden_state, all_router_logits
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        hidden_state, all_router_logits = self.moe(self.ln_2(x))
        x = x + hidden_state
        return x
